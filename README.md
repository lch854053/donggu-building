...
Figure-Ground의 구조 모드는 D199 건축물대장의 `건축물구조명` 기재값을 `structure` 속성으로 표시합니다. 현재 구조명 확인은 14,036건(전체 24,012건의 58.45%)이며, `벽돌구조`, `철근콘크리트구조`, `일반목구조`, `블록구조`, `경량철골구조` 상위 5종이 확인된 구조명의 96.90%를 차지합니다. 상위 5종 외의 구조명은 기타, 미기재 구조명은 미확인으로 표시합니다.
